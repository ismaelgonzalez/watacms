<div class="videos form">
	...
	?>
	<fieldset>
		<legend><?php echo __('Admin Agregar Video'); ?></legend>
		<?php
		echo $this->Form->input('id', array(
			'default' => $video['Video']['id']
		));
		echo $this->Form->input('video', array(
			'label' => array('text' => 'Video', 'class' => 'control-label my-label col-lg-2'),
			'type'  => 'text',
			'class' => 'form-control',
		...
		echo $this->Form->input('published_date', array(
			...
			'type' => 'text',
			'between' => '<div class="col-lg-2">',
			'default' => $video['Video']['published_date'],
		));
		$time = $this->Timeoptions->getTimeOptions();

		$minute = date('i', strtotime($video['Video']['published_time']));
		if ($minute != '00') {
			$default_time = 0;
			echo $this->Form->hidden('time_is_set', array('default' => 1));
		} else {
			$default_time = date('H', strtotime($video['Video']['published_time']));
		}
		...
		<div class="form-group">
			...
			<div class="col-lg-4">
				<label for="VideoIsPublished1">Si</label>
				<input type="radio" name="data[Video][is_published]" id="VideoIsPublished1" value="1" checked="checked">
				<label for="VideoIsPublished0">No</label>
				<input type="radio" name="data[Video][is_published]" id="VideoIsPublished0" value="0">
			</div>
		</div>
	</fieldset>
	<?php
...
<script type="text/javascript">
	$(function () {
		$('#VideoPublishedDate').datepicker({dateFormat:'dd-mm-yy'});
